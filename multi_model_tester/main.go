...
func sendReqest(URL *string, req *pb.ModelInferRequest) {
	// Directly send request to the gRPC server.
	// Needs to import pb.go with grpc client interface which can be generated with the following
	...
	if err != nil {
		log.Fatalf("Error processing InferRequest for %v: %v", req.ModelName, err)
	}
