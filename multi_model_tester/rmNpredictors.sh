...
then
  ...
  echo "     parallel - number of predictors to be concurrelty removed"
  echo "     prefix - prefix to use in the predictor name"
  echo "     startID - the starting ID of a prefix-ID"
  echo "     endID  - the ending ID of the predictor to remove"
  echo "     script - one of the scripts to remove a predictors"
  exit
fi
